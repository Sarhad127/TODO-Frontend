...
function TodoColumn({
    ...
    const moveTodoWithinColumn = async (fromIndex, toIndex, column) => { /* TODO moves task within same column */
        ...
        try {
            ...
            const response = await fetch('email-verification-production.up.railway.app/tasks/reorder', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`,
    ...
    const moveTodo = async (fromIndex, fromColumn, toColumn) => {
        ...
        try {
            ...
            const response = await fetch(`email-verification-production.up.railway.app/tasks/move/${taskId}?newColumnId=${newColumnId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`,
