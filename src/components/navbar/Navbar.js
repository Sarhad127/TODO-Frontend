...
const Navbar = ({ onBoardSelect }) => {
    ...
    useEffect(() => {
        const fetchBoards = async () => {
            try {
                ...
                const response = await fetch('email-verification-production.up.railway.app/api/boards', {
                    headers: {
    ...
    const handleBoardClick = async (position) => {
        try {
            ...
            const boardResponse = await fetch(`email-verification-production.up.railway.app/api/boards/${position}`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                },
            });
            ...
            const usersResponse = await fetch(`email-verification-production.up.railway.app/api/boards/${boardData.id}/users`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                },
            });
    ...
    const renameBoard = async (boardId, newTitle) => {
        try {
            const token = localStorage.getItem('token') || sessionStorage.getItem('token');
            const response = await fetch(`email-verification-production.up.railway.app/api/boards/${boardId}/title`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
    ...
    const deleteBoard = async () => {
        try {
            ...
            const response = await fetch(`email-verification-production.up.railway.app/api/boards/${board.id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            ...
            const boardsResponse = await fetch('email-verification-production.up.railway.app/api/boards', {
                headers: {
                    'Authorization': `Bearer ${token}`,
                },
            });
    ...
    const handleAddFriend = async () => {
        ...
        try {
            const token = localStorage.getItem('token') || sessionStorage.getItem('token');
            const response = await fetch('email-verification-production.up.railway.app/api/invitations/invite', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
    ...
    const leaveGroup = async () => {
        ...
        try {
            const token = localStorage.getItem('token') || sessionStorage.getItem('token');
            const response = await fetch(`email-verification-production.up.railway.app/api/boards/${currentBoardId}/leave`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
